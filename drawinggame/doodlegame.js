...
const keywords = [
  ...
  "ice cream", "snowflake", "cookie", "beach", "leg", 'camera', 'star'
];
...
function preloadDoodleAssets() {
  ...
  brushsound = loadSound('assets/paintstroke.mp3')
}
...
function playDoodleGame() {
  ...
  if (doodlePhase !== 2 && mouseIsPressed &&
    ...
    if (!isBrushing) {
      ...
      brushsound.play();     // play one shot
      isBrushing = true;
    }
  } else {
    // stop brush SFX when the mouse is released or leaves the area
    if (isBrushing) {
      // brushsound.stop();
      isBrushing = false;
    }
